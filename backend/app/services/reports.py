...
def get_report(report_type: str, from_: str, to: str) -> dict | list:
    ...
    if report_type == "dashboard":
        ...
        range_revenue = db.table("appointments").select("price, discount").eq("status", "completed").eq("payment_status", "paid").gte("starts_at", from_).lte("starts_at", to).execute().data or []
        range_orders = db.table("orders").select("total_amount").not_.in_("status", ["cancelled", "refunded"]).gte("created_at", from_).lte("created_at", to).execute().data or []
